...
export default function handler(req, res) {
    ...
    if (apiKey !== 'noonhub-secure-2024') {
        return res.status(401).json({ error: 'Unauthorized - Invalid API key' });
    }

    // Check user agent
    const userAgent = req.headers['user-agent'];
    if (!userAgent || !userAgent.includes('NoonHub-Loader')) {
        return res.status(403).json({ error: 'Forbidden - Invalid user agent' });
    }

    // Your actual script content (the one you provided)
    const scriptContent = `task.wait(0.1)
...
subtitle.Text = "https://discord.gg/fDMN3pMfY6"
...
UserInputService.InputBegan:Connect(function(input, gpe)
    ...
    res.setHeader('X-Encryption', 'AES-256');
